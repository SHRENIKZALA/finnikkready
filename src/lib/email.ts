...
export async function sendEmail(input: SendEmailInput): Promise<void> {
  ...
  const from = process.env.EMAIL_FROM ?? 'Finnikk Business OS <zalashrenik1811@gmail.com>'
...
export async function sendVerificationEmail(input: {
  ...
  await sendEmail({
    to: input.to,
    subject: 'Verify your Finnikk Business OS email',
    html: emailShell(
      'Confirm your email',
      'Confirm your email address to secure your Finnikk Business OS account.',
...
export async function sendPasswordResetEmail(input: {
  ...
  await sendEmail({
    to: input.to,
    subject: 'Reset your Finnikk Business OS password',
    html: emailShell(
      'Reset your password',
...
export async function sendInvitationEmail(input: {
  ...
  await sendEmail({
    to: input.to,
    subject: `You're invited to ${input.companyName} on Finnikk Business OS`,
    html: emailShell(
